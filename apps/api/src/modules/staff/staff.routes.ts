...
router.post('/:id/assignments', async (req, res) => {
  try {
    ...
    const existingAssignment = await prisma.staffGroupAssignment.findFirst({
      where: { schoolId, staffId, groupId, status: 'ACTIVE' },
    })

    const assignment = existingAssignment
      ? await prisma.staffGroupAssignment.update({
        where: { id: existingAssignment.id },
        data: { assignmentType: assignmentType || 'MAIN_TEACHER', status: 'ACTIVE' },
      })
