...
export default function HealthPage() {
  ...
  return (
    <div className="p-4 md:p-8 animate-in min-h-screen bg-[#FDFCFE]">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
        <div>
          ...
          <p className="text-gray-500 font-medium">Controle de medicações, alergias e cuidados especiais.</p>
        </div>
        <div className="relative w-full md:w-80">
          ...
          <input 
            ...
            className="w-full pl-12 pr-6 py-4 bg-white border border-gray-100 rounded-[2rem] shadow-sm focus:ring-2 focus:ring-primary/20 outline-none font-bold text-sm transition-all"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-8 rounded-[3rem] border border-gray-100 shadow-sm overflow-hidden relative">
            ...
            <h3 className="font-black text-gray-900 uppercase tracking-widest text-xs mb-6">Medicações de Hoje</h3>
            <div className="flex items-end gap-3">
              <span className="text-5xl font-black text-primary">{medications.length}</span>
              <span className="text-gray-400 font-bold mb-2 uppercase text-[10px]">Ativas</span>
            </div>
          </div>
          ...
          <div className="bg-amber-50 p-8 rounded-[3rem] border border-amber-100 relative overflow-hidden">
            ...
            <h3 className="font-black text-amber-900 uppercase tracking-widest text-xs mb-4">Atenção!</h3>
        ...
        <div className="lg:col-span-3 space-y-6">
          {loading ? (
            ...
            <div className="bg-white p-20 rounded-[3rem] border-2 border-dashed border-gray-100 text-center">
              ...
              <p className="text-gray-400 font-bold uppercase tracking-widest text-sm">Nenhuma medicação ativa</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex items-center gap-4 mb-6">
                    <div className="relative">
                      {med.child.photoUrl ? (
                        <img src={med.child.photoUrl} className="w-14 h-14 rounded-2xl object-cover" alt="" />
                      ) : (
                        <div className="w-14 h-14 rounded-2xl bg-lime-100 flex items-center justify-center text-primary font-black text-xl">
                    ...
                    <div>
                      <h3 className="font-black text-gray-900 leading-tight">{med.child.fullName}</h3>
                      <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{med.child.group?.name || 'Sem turma'}</p>
                    </div>
                  </div>

                  <div className="bg-gray-50 rounded-3xl p-5 mb-6">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-xs font-black text-gray-400 uppercase tracking-widest">Medicamento</span>
                      <span className="bg-rose-100 text-rose-600 text-[10px] font-black px-2 py-0.5 rounded-full uppercase">{med.frequency}</span>
                    </div>
                    <p className="text-lg font-black text-gray-800">{med.name}</p>
                    <p className="text-sm font-bold text-gray-500 mt-1 flex items-center gap-2">
                      <Thermometer size={14} className="text-primary" />
                      Dosagem: <span className="text-gray-900">{med.dosage}</span>
                    </p>
                  </div>

                  <div className="flex items-center justify-between">
                    <div className="flex flex-col">
                      <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Última Dose</span>
            ...
            >
              <div className="p-8 bg-gray-50 border-b border-gray-100 flex justify-between items-center">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h2 className="text-xl font-black text-gray-900 tracking-tight">Registrar Dose</h2>
                    <p className="text-xs font-bold text-gray-400 uppercase tracking-widest">{selectedMed.child.nickname || selectedMed.child.fullName}</p>
                  </div>
                </div>
                <button 
                  onClick={() => setShowModal(false)}
              ...
              <form onSubmit={handleAdminister} className="p-8 space-y-6">
                <div className="bg-blue-50 p-6 rounded-[2rem] border border-blue-100 flex items-start gap-4">
                  ...
                  <div>
                    <p className="text-xs font-black text-blue-900 uppercase tracking-widest mb-1">Prescrição Original</p>
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase ml-2">Confirmar Dosagem</label>
                  <input 
                    type="text" 
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-gray-400 uppercase ml-2">Observações / Reações</label>
                  <textarea 
                    rows={3}
