...
export default function TeacherHomePage() {
  ...
  return (
    <div className="space-y-6">
      <section>
        <h1 className="text-2xl font-black text-foreground">Olá, Professor(a)!</h1>
        <p className="text-muted-foreground font-medium">Selecione uma turma para iniciar os registros de hoje.</p>
      </section>

      {loading ? (
        <div className="grid gap-4 sm:grid-cols-2">
          {[1, 2].map((i) => (
            <div key={i} className="h-32 animate-pulse rounded-xl bg-accent shadow-sm border border-border"></div>
          ))}
        </div>
      ) : groups.length === 0 ? (
        <div className="card flex flex-col items-center justify-center p-12 text-center border-dashed">
          ...
          <p className="text-xs text-muted-foreground font-medium mt-1">Você não está vinculado a nenhuma turma no momento.</p>
        </div>
      ) : (
        <div className="grid gap-4 sm:grid-cols-2">
          {groups.map((group) => (
            ...
            >
              ...
              <div className="mt-5">
                ...
                <p className="text-sm text-muted-foreground font-medium mt-0.5">{group._count.children} alunos matriculados</p>
              </div>
